      ...
      <Slide delay={0.18}>
        <div className="flex flex-col gap-y-24">
          {jobs.map((data: any) => (
            <div
              key={data._id}
              className="grid lg:grid-cols-3 grid-cols-1 gap-x-8 gap-y-8 border-t dark:border-zinc-800 border-zinc-200 pt-10"
            >
              {/* Left */}
              <div className="flex flex-col gap-y-4">
                <h3 className="text-2xl sm:text-3xl font-normal dark:text-zinc-200 text-zinc-800 tracking-tight">
                  {data.name}
                </h3>
                <p className="text-sm dark:text-zinc-400 text-zinc-600">{data.tagline}</p>
              ...
              <div className="flex flex-col gap-y-4">
                {[
                  ...
                  <div key={label} className="grid grid-cols-[100px_1fr] gap-x-4">
                    <span className="text-sm dark:text-zinc-500 text-zinc-400">{label}</span>
                    <span className="text-sm dark:text-zinc-300 text-zinc-700">{value}</span>
                  </div>
                ))}
                <div className="grid grid-cols-[100px_1fr] gap-x-4">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-sm dark:text-zinc-300 text-zinc-700 hover:dark:text-white hover:underline underline-offset-4 decoration-zinc-500 break-all"
                  >
                    {data.website} ↗
                  </a>
                </div>
